    ...
    while (agt->rip[rc]) {
        ...
        if (agt->sock < 0) {
            ...
            if (agt->rip[rc] == NULL) {
                ...
                verbose("%s : INFO: will sleep for %d secs .... ",ARGV0,attempts); 
                sleep(attempts);
                rc = 0;
            }
        } else {
    ...
    while (1) {
        ...
        while (((recv_b = recv(agt->sock, buffer, OS_MAXSTR,
            ...
            if (recv_b <= 0) {
                 ...
                 */
                attempts++;
                verbose("%s : INFO: will sleep for %d secs ....(waiting for reply) ",ARGV0,attempts); 
                sleep(attempts);
        ...
        if (agt->rip[1]) {
            ...
            if (agt->rip_id == curr_rip) {
                verbose("%s : INFO: will sleep for %d secs .... server: '%s' ",ARGV0,g_attempts,
                        agt->rip[agt->rip_id + 1] != NULL ? agt->rip[agt->rip_id + 1] : agt->rip[0]); 
                sleep(g_attempts);
